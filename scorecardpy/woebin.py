...
def woebin2_breaks(dtm, breaks, spl_val):
    ...
    vals = dtm["value"]
    dtm["value"] = vals.apply(pd.to_numeric)
    print(vals.values)
    # binning
    if is_numeric_dtype(dtm['value']):
        # best breaks
        ...
        # cut
        labels = ['[{},{})'.format(bstbrks[i], bstbrks[i+1]) for i in range(len(bstbrks)-1)]
        dtm.loc[:,'bin'] = pd.cut(dtm['value'], bstbrks, right=False, labels=labels, include_lowest=True)
        binning = dtm.groupby(['variable','bin'])['y'].agg([n0, n1])\
          .reset_index().rename(columns={'n0':'good','n1':'bad'})
        # check empty bins for unmeric variable
...
def woepoints_ply1(dtx, binx, x_i, woe_points):
    ...
    print(binx);
    vals = dtx[x_i]
    dtx[x_i] = vals.apply(pd.to_numeric)
    
    # dtx
    ## cut numeric variable
    if is_numeric_dtype(dtx[x_i]):
        print("inside numeric_dtype woepoints_ply1")
    ...
    #print(dtx)
    #dtx = dtx.cat.add_categories("missing").fillna('missing').assign(rowid = dtx.index)
    # rename binx
